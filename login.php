<!DOCTYPE html>
<html lang="en">
<?php

require_once("./layout/header.php");
session_start();
session_destroy();
?>

<body>
  <div class="text-center  " data-new-gr-c-s-check-loaded="14.1056.0" data-gr-ext-installed="">
    <main class="form-signin w-25 mx-auto mt-5">
      <form method="POST" action="controllers/auth-controller.php">
        <img class="mb-4" src="data:image/png;base64," alt="" width="72" height="57">
        <h1 class="h3 mb-3 fw-normal">Please sign in</h1>
        <?php
        if (isset($_GET['status'])) {
          $status = $_GET['status'];
          $msg = $_GET['msg'];
          echo "<div  style='padding: 10px 10px;text-align: center;margin: 5px;color: rgb(232, 65, 65);'>$msg</div>";
        }
        ?>
        <div class="form-floating">
          <input name="username" type="text" class="form-control" id="floatingInput" placeholder="">
          <label for="floatingInput">username</label>
        </div>
        <div class="form-floating">
          <input name="password" type="password" class="form-control" id="floatingPassword" placeholder="">
          <label for="floatingPassword">Password</label>
        </div>

        <div class="checkbox mb-3">
          <label>
            <input type="checkbox" value="remember-me"> Remember me
          </label>
        </div>
        <button type="submit" value="100" name="log" class=" w-100 btn btn-lg btn-primary">Log in</button>
        <a href="./signup.php" class=" mt-5 text-primary reg">Register a new account</a>
        <p class="mt-5 mb-3 text-muted">© 2020–2021</p>
      </form>
    </main>

  </div>
</body>

</html>